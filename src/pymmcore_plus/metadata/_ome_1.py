...
def _create_position_to_image_mapping(
    ...
    """Create a mapping from position index to sequential image ID.

    Returns a mapping where keys are position indices from the MDAEvent,
    and values are the sequential image IDs (0, 1, 2, ...).
    """
    # Sort position groups by (p_index, g_index) to ensure consistent ordering
    sorted_position_items = sorted(
        position_groups.items(),
        key=lambda item: (item[0].p_index, item[0].g_index or 0),
    )

    position_to_image_mapping: dict[int, int] = {}
    for image_idx, (_position_key, position_frames) in enumerate(sorted_position_items):
        if position_frames:
            mda_event = _extract_mda_event(position_frames[0])
            if mda_event is not None:
                position_index = mda_event.index.get("p", 0)
                # Map the acquisition position index to sequential image ID
                # For grids, multiple image IDs may map to the same position index
                # We use the first one encountered for each position
                if position_index not in position_to_image_mapping:
                    position_to_image_mapping[position_index] = image_idx
...
def _build_ome_images(
    ...
    Images are assigned sequential integer IDs (0, 1, 2, ...) following OME standards,
    while maintaining descriptive names based on position and grid indices.
    """
    # Sort position groups by (p_index, g_index) to ensure consistent ordering
    sorted_position_items = sorted(
        position_groups.items(),
    ...
    images = []
    for image_idx, (position_key, position_frames) in enumerate(sorted_position_items):
        # Use sequential integer ID for OME compliance
...
def _build_ome_plate(
    plate_plan: useq.WellPlatePlan, position_to_image_mapping: dict[int, int]
) -> Plate:
