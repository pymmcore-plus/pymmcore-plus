...
def _build_linux(
    ...
    for dep in ("autoconf", "automake"):
        if not shutil.which(dep):
            print(f"{dep!r} is required but not found. Please install it first.")
            return

    # check boost with dpkg -l | grep
    output = subprocess.run(["dpkg", "-l"], capture_output=True)
    if "libboost-dev" not in output.stdout.decode():
        print(
            "libboost-dev is required but not found. "
...
def build(dest: Path, repo: str = MM_REPO, overwrite: bool | None = None) -> None:
    ...
    if system == "Linux":
        return _build_linux(dest, repo, overwrite)

    print(f"Sorry, {system} {machine} not supported at this time.")
